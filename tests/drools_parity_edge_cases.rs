...
fn load_program_and_context(rule_file: &str, applicant_file: &str) -> (RuleProgram, FactContext) {
    ...
    let app_content = fs::read_to_string(&app_path).unwrap_or_else(|e| {
        panic!(
            "Failed to read applicant file {}: {}",
            app_path.display(),
            e
        )
    });
    ...
    let program =
        RuleProgram::from_yaml_str(&rule_content).expect("Failed to parse rule program YAML");
    let app_val: serde_json::Value =
        serde_yaml::from_str(&app_content).expect("Failed to parse applicant YAML");
...
fn test_tc01_skill_transferability_and_clb9() {
    let (program, context) = load_program_and_context(
    ...
    let transferability = report
        .category_scores
        .iter()
        .find(|c| c.category == "skill_transferability")
        .unwrap();
    assert_eq!(
        transferability.capped_points, 100.0,
        "Transferability should be at maximum cap of 100"
    );

    let core = report
        .category_scores
        .iter()
        .find(|c| c.category == "core_human_capital")
        .unwrap();
    assert!(
        core.capped_points >= 400.0,
        "Core Human Capital should be >= 400"
    );
}
...
fn test_tc02_married_phd_provincial_nomination() {
    let (program, context) = load_program_and_context(
        "canada_crs_express_entry.yaml",
        "tc02_married_phd_researcher.yaml",
    );
    let engine = Engine::new(program);
    let report = engine.evaluate(&context).expect("Evaluation failed");
    ...
    let additional = report
        .category_scores
        .iter()
        .find(|c| c.category == "additional_points")
        .unwrap();
    assert_eq!(
        additional.capped_points, 600.0,
        "Provincial nomination should award 600 points"
    );
}
...
fn test_tc03_subcategory_cap_overflow_truncation() {
    let (program, context) = load_program_and_context(
        "canada_crs_express_entry.yaml",
        "tc03_cap_overflow_tradesperson.yaml",
    );
    let engine = Engine::new(program);
    let report = engine.evaluate(&context).expect("Evaluation failed");
    ...
    let transferability = report
        ...
        .unwrap();
    assert!(
        transferability.raw_points >= 150.0,
        "Raw transferability should be >= 150"
    );
    assert_eq!(
        transferability.capped_points, 100.0,
        "Capped transferability must strictly be 100.0"
    );
    assert!(
        transferability.is_capped,
        "Category should be flagged as is_capped = true"
    );
}
...
fn test_tc04_multi_fact_relational_join_failure() {
    ...
    assert!(
        report
            ...
            .iter()
            .any(|r| r.contains("Job Offer from Licensed Sponsor") || r.contains("sponsor")),
...
fn test_tc05_activation_group_xor_mutual_exclusion() {
    ...
    let tradeable = report
        .category_scores
        .iter()
        .find(|c| c.category == "tradeable_criteria")
        .unwrap();
    assert_eq!(
        tradeable.capped_points, 20.0,
        "Tradeable points should be exactly 20 (XOR mutual exclusion)"
    );

    // Ensure Option A fired and Option B was skipped
    let fired_ids: Vec<&str> = report
        .fired_rules
        .iter()
        .map(|r| r.rule_id.as_str())
        .collect();
    assert!(
        fired_ids.contains(&"uk_tradeable_option_a_salary"),
        "Option A must fire"
    );
    assert!(
        !fired_ids.contains(&"uk_tradeable_option_b_stem_phd"),
...
fn test_tc06_hard_age_gate_disqualification() {
    let (program, context) = load_program_and_context(
        ...
        "tc06_australia_age_barred.yaml",
    );
    let engine = Engine::new(program);
    let report = engine.evaluate(&context).expect("Evaluation failed");
    ...
    assert!(
        !report.is_eligible(),
        "Age 46 candidate must be disqualified for Australia 189"
    );
    assert!(
        report
            .ineligibility_reasons
            .iter()
            .any(|r| r.contains("45 and over")),
        "Should contain age 45+ barrier reason"
    );
}
...
fn test_tc07_temporal_cep_expiration() {
    let (program, context) = load_program_and_context(
        ...
        "tc07_temporal_expired_ielts.yaml",
    );
    let engine = Engine::new(program);
    let report = engine.evaluate(&context).expect("Evaluation failed");
    ...
    assert!(
        !report.is_eligible(),
        "Expired IELTS test (820 days old) must fail recency gate"
    );
    assert!(
        report
            .ineligibility_reasons
            .iter()
            .any(|r| r.contains("730 days")),
        "Should cite expiration limit"
    );
}
...
fn test_tc10_forall_language_floor_failure() {
    let (program, context) = load_program_and_context(
        "edge_cases_drools_parity_suite.yaml",
        "tc10_forall_language_fail.yaml",
    );
    let engine = Engine::new(program);
    let report = engine.evaluate(&context).expect("Evaluation failed");
    ...
    let fired_ids: Vec<&str> = report
        ...
        .collect();
    assert!(
        !fired_ids.contains(&"ec_10_forall_language_floor"),
        "Universal quantifier rule must NOT fire"
    );
}
...
fn test_modular_directory_rule_loading() {
    ...
    let program =
        RuleProgram::from_path("rules/canada_crs").expect("Failed to load modular rule directory");
    assert_eq!(program.id, "canada_crs_express_entry");
    assert!(
        !program.categories.is_empty(),
        "Categories should be merged from directory files"
    );
    assert!(
        program.rules.len() >= 10,
        "Rules should be merged from all modular files in directory"
    );

    let app_content = std::fs::read_to_string("applicants/tc01_tech_lead_single.yaml")
        .expect("Failed to read applicant");
    let app_val: serde_json::Value =
        serde_yaml::from_str(&app_content).expect("Failed to parse applicant");
    let context = FactContext::from_value(app_val);

    let engine = Engine::new(program);
    let report = engine
        .evaluate(&context)
        .expect("Evaluation from modular directory failed");

    assert!(report.is_eligible());
    assert!(
        report.total_score >= 580.0,
        "Score should match single file evaluation (got {})",
        report.total_score
    );
}
